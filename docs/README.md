...
## Version
Currently set up for `python 3.14.5` with `Django 6.0.5` and `Django Rest Framework 3.17.1`.
...
**It's highly advised that you have version control set up and are working on a clean tree when you start the update.** Updating the boilerplate will modify files in the project through adding, removing, and merging files; when going through major version updates, it's very likely that some automated changes may be wrong and break things. The recommended process for updating involves creating a new branch, performing the update, fixing any merge conflicts generated by the update, re-building the project, running the test suite again, and then carefully reviewing the pull request from the new branch back to the original branch.
...
### Before 3.0.0
The auto-updater had some issues dealing with changes that involved _entire_ folders, and this version has a case as such (`/requirements` replaced with requirements in `pyproject.toml` files).

To update from a version below `3.0.0`, replace the contents of the CLI's `__init__.py` with the `3.0.0`'s version of that file, and the contents of `update.py` with those of the `3.0.0`'s version `cli/src/update.py`. You may need to adjust imports (remove `cli.src` from them) to get it to work. Then running `python make.py update` as usual should work.
